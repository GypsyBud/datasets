...
@attr.s(frozen=True)
class _FileInstructionsOutput(object):
  """Output of the file instruction."""
...
def make_file_instructions(name, split_infos, instruction):
  ...
  Returns:
    The files instructions output containing:
     * List[dict(filename, skip, take)], the files instructions. The filenames
       contains the relative path, not absolute
...
def _make_file_instructions_from_absolutes(
  ...
  """Returns the files instructions from the absolute instructions."""
  # For each split, return the files instruction (skip/take)
  all_file_instructions = []
  num_examples = 0
  for abs_instr in absolute_instructions:
    ...
    file_instructions = _sharded_files.get_read_instructions(
        from_, to, filenames, shard_lengths)
    all_file_instructions.extend(file_instructions)
  return _FileInstructionsOutput(
      num_examples=int(num_examples),  # int() due to proto shard_length `long`
      file_instructions=all_file_instructions,
  )
